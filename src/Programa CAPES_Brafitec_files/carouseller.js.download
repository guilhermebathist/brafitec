(function($) {
	$.fn.carouseller = function(options) {
		...
		count = obj.find('[class*="sld__"]').length,
		sens = options.sens,
		...
		get_el_width = function(){
			return obj.find('[class*="sld__"]').eq(-1).outerWidth(true);
		},
		...
		left = function(e, result){
			...
			if ( count > can_view) {
				if (e.type == 'touchend') {
					...
					obj.find('[class*="sld__"]').eq(-1).prependTo(list);
					list.css({ left: '-'+t+'px' });
					list.stop().animate({
						left: '0px'
					}, options.scrollSpeed, options.easing, function() {
			...
			if ( count > can_view) {
				if (e.type == 'touchend') {
					list.animate({
						...
						obj.find('[class*="sld__"]').eq(0).appendTo(list);
						obj.find('[class*="sld__"]').eq(0).appendTo(list);
						list.css({ left: '0px' });
